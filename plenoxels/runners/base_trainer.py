...
class BaseTrainer(abc.ABC):
    ...
    def train_step(self, data, **kwargs) -> bool:
        self.model.train() #Put model in training state
        ...
        if self.global_step % 1000 == 0:
            with torch.no_grad():   
                log = {}
                if self.model.what['field']:
                    ms_grids = self.model.field.grids()
                else:
                    ms_grids = self.model.field.grids
                for scale_id, plane_group in enumerate(ms_grids):
                    for plane_id, plane in enumerate(plane_group):
                        # Take mean features of planes # (1, M, H, W) -> (H, W)
                        plane = plane.squeeze(0).mean(0)
                        img = plane
                        image = wandb.Image(img)
                        ext = 'spacetime' if plane_id in [2,4,5] else 'space'
                        log[f'{ext}Plane vs DeformedPlane/{scale_id}_{plane_id}'] = image
            
                wandb.log(log, step=int(self.global_step))     
    ...
    def calc_metrics(self, preds: torch.Tensor, gt: torch.Tensor):
        if gt.shape[-1] == 4:
            # Replace transparent values with white background
            white_rgb = gt[..., :3] * gt[..., 3:] + (1.0 - gt[..., 3:])

            # Mophological dilation of alpha mask
            kernel = np.ones((3,3), np.float64) 
            white_mask = cv2.dilate(gt[..., 3].cpu().numpy(), kernel, iterations=20)

            psnr_f, psnr_b =  metrics.psnr_masked(preds, white_rgb, white_mask)
          
        else:
            white_rgb = gt
            psnr_f,psnr_b = 0., 0.
            
        err = (white_rgb - preds) ** 2


        return {
            "mse": torch.mean(err),
            "psnr": metrics.psnr(preds, white_rgb),
            "ssim": metrics.ssim(preds, white_rgb),
            "ms-ssim": metrics.msssim(preds, white_rgb),
            "psnr-foreground": psnr_f,
            "psnr-background":psnr_b,
    ...
    def evaluate_metrics(self,
        ...
        if wandb.run is not None:
            image = wandb.Image(out_img_np)
            depthim = wandb.Image(out_depth_np)
            wandb.log({'image':image, 'depth':depthim})
    ...
    def get_save_dict_modelonly(self):
        return {
            "model": self.model.state_dict(),
        }
    
    def save_model_only(self):
        torch.save(self.get_save_dict_modelonly(), 'model_alone.pth')
