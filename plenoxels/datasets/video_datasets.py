...
def dynerf_isg_weight(imgs, median_imgs, gamma):
    ...
    print("About to consume all your RAM ...")
    # subtractor = median_imgs[:, None, ...].float().div_(255.0).cuda()
    # imgs = imgs.cuda()

    # print(imgs.shape, num_cameras, h, w, c)
    # print(imgs.device, subtractor.device)

    # Original func provided by K-Planes which throws kill with no error
    squarediff = (
...
def dynerf_ist_weight(imgs, num_cameras, alpha=0.1, frame_shift=25):  # DyNerf uses alpha=0.1
    assert imgs.dtype == torch.uint8
    print('dynerf_ist_weight')
    N, h, w, c = imgs.shape
    frames = imgs.view(num_cameras, -1, h, w, c).float()#.float()  # [num_cameras, num_timesteps, h, w, 3]
    max_diff = None
