...
def psnr_masked(rgb, gts, mask):
    """Calculate the PSNR metric with alpha mask - return foreground and background psnr
    ...
    # Mask out the transparent regions and invert mask to seperate foreground and background
    gts_foreground = gts*mask
    gts_background = gts*np.absolute(1.-mask) # inverted mask

    rgb_foreground = rgb*mask
    rgb_background = rgb*np.absolute(1.-mask)
    
    # Count the number of pixels to do mse for masked regions
    mask_size = np.count_nonzero(mask)
    invmask_size = mask.size - mask_size

    # Do mse
    mse_foreground = np.sum( (rgb_foreground - gts_foreground) ** 2 )/mask_size
    mse_background = np.sum( (rgb_background - gts_background) ** 2 )/invmask_size

    # Calc psnr
    psnr_f = 10 * math.log10(1.0 / mse_foreground)
    psnr_b = 10 * math.log10(1.0 / mse_background)
        
    return psnr_f, psnr_b
