...
from plenoxels.models.old_density_field import KPlaneDensityField as KPDF
from plenoxels.models.kplane_field import KPlaneField as KPF
...
from plenoxels.models.grids_field import WaveletField
...
class LowrankModel(nn.Module):
    def __init__(self,
                 ...
                 use_wavelet_psn:bool=True,
                 use_wavelet_field:bool=True,
                 fusion:str='None',
        ...
        self.what = {'field':use_wavelet_field, 
                     'psn':use_wavelet_psn
                     }
        ...
        if use_wavelet_field:
            self.field = WaveletField(
                aabb,
        ...
        self.density_fns = []
        self.num_proposal_iterations = len(proposal_net_args_list)
        ...
        # Select the type of density field
        if use_wavelet_psn: # Our density field (mini planes)
            if use_same_proposal_network:
                ...
                network = KPlaneDensityField(
                    ...
                    regularise_wavelet_coeff=grid_config[1]['regularise_wavelet_coeff'],
                    concat_features_across_scales=grid_config[1]['concat_features_across_scales'],
                    wave=grid_config[1]['wave'],
                    wave_mode=grid_config[1]['wave_mode'],
                    fusion=fusion,
                    is_static = self.is_static,
                    cache_planes=cache_planes,
                    **prop_net_args
        ...
        # print('Proposal Warmul', self.proposal_warmup)
        # print('Proposal UpdateEvery ', self.proposal_update_every)
    ...
    def forward(self, rays_o, rays_d, bg_color, near_far: torch.Tensor, timestamps=None,):
        ...
        # if self.is_static:
        #     timestamps = torch.zeros(rays_d.shape[0]).to(rays_d.device)
