...
from runners.wandb_writer import Writer
...
def main():
    ...
    # config['use_wavelet_psn'] = bool(int(config['use_wavelet_psn']))
    # config['use_wavelet_field'] = bool(int(config['use_wavelet_field']))
    # config['linear_decoder_layers'] = int(config['linear_decoder_layers'])
    # config['l1_time_planes'] = float(config['l1_time_planes'])
    # config['l1_time_planes_proposal_net'] = float(config['l1_time_planes_proposal_net'])
    # config['time_smoothness_weight'] = float(config['time_smoothness_weight'])
    # config['time_smoothness_weight_proposal_net'] = float(config['time_smoothness_weight_proposal_net'])
    # config['plane_tv_weight'] = float(config['plane_tv_weight'])
    # config['plane_tv_weight_proposal_net'] = float(config['plane_tv_weight_proposal_net'])
    ...
    if validate_only:
        trainer.validate(is_compressing=True)
    elif render_only:
        render_to_path(trainer, extra_name="")
    elif spacetime_only:
        decompose_space_time(trainer, extra_name="")
    else:
        writer = Writer(config=config, name=config['expname'], proj=config['wandbproject'])
        writer.run(trainer)
